...
object LevelCalculator {
    ...
    fun getDifficultyForLevel(level: Int): Float {
        ...
        return when {
            // 난이도 1 (쉬움): 1~5단계 - 색상 차이 중간 (0.35~0.25)
            level <= 5 -> 0.35f - (level - 1) * 0.025f    // 0.35 → 0.25

            // 난이도 2 (보통): 6~10단계 - 색상 차이 작음 (0.25~0.15)
            level <= 10 -> 0.25f - (level - 6) * 0.02f  // 0.25 → 0.15

            // 난이도 3 (어려움): 11~15단계 - 색상 차이 매우 작음 (0.15~0.09)
            level <= 15 -> 0.15f - (level - 11) * 0.012f // 0.15 → 0.09

            // 난이도 4 (고급): 16~20단계 - 극미세한 차이 (0.09~0.05)
            level <= 20 -> 0.09f - (level - 16) * 0.008f // 0.09 → 0.05

            // 난이도 5 (전문가): 21~25단계 - 거의 구별 불가 (0.05~0.03)
            level <= 25 -> 0.05f - (level - 21) * 0.004f // 0.05 → 0.03

            // 난이도 6 (마스터): 26~30단계 - 최고 난이도 (0.03~0.015)
            else -> 0.03f - (level - 26) * 0.003f       // 0.03 → 0.015
        }
    }
